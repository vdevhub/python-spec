...
class Recipe(Base):
  ...
  def __str__(self):
    ...
    recipe_string = "\nRecipe ID " + str(self.id) + " - " + self.name + \
                    "\n" + "-"*30 + \
...
def create_recipe():
  ...
  while True:
    ...
    if len(name) > 50:
      ...
      print("Error: Name must contain only alphabethical characters. Please try again.")
    else:
      break
...
def search_by_ingredients():
  ...
  selected_ingr_indices_str = input("Type the numbers of the ingredients to be searched for, separated by spaces: ")
  ...
  if any(i < 0 or i >= len(all_ingredients) for i in selected_ingr_indices):
    print("Error: Invalid selection. Enter numbers from the displayed options only.")
    return None
...
def edit_recipe():
  ...
  if not selected_recipe_id.isnumeric() or int(selected_recipe_id) not in [recipe.id for recipe in results]:
    print("Error: You must select a recipe by its ID. See the listed recipes with their respective IDs and try again.")
    return None
  ...
  attribute_to_edit_sel = input("Select the attribute you'd like to edit by its number - 1, 2, or 3: ")
  if not attribute_to_edit_sel in ["1", "2", "3"]:
    print("Error: Incorrect input. You must select one of the three attributes by its number.")
    return None
  ...
  if attribute_to_edit_sel == "1":
    ...
    if len(new_name) > 50 or not new_name.isalpha():
      print("Error: The name must have max 50 characters and contain only alphabetical characters.")
      return None
...
def delete_recipe():
  ...
  selected_recipe_id = int(selected_recipe_id)
  recipe_to_delete = session.query(Recipe).filter(Recipe.id == selected_recipe_id).one()

  # Deletion confirmation
  user_choice = input("Do you want to delete the recipe " + str(recipe_to_delete.id) + " " + recipe_to_delete.name + "? Enter y or n: ").lower()
...
def main_menu():
  ...
  while (choice != 'quit'):
    ...
    if choice == '1':
      ...
      print("Your choice doesn't look correct. Please pick a number from the provided menu or type 'quit' to quit the app.")
